	...
	return (
		<nav className={styles["container"]}>
			...
			<ul className={styles["links"]}>
				...
				<li className={styles["item"]}><a href="/income" className={styles["link"]}>Income</a></li>
			</ul>
		</nav>
	);
}
...
export function Month({ currentMonth, isNegative, toggleMonthModal }: MonthProps) {
	return (
		<button className={`${styles["month-container"]} ${styles["change-month"]} ${styles[isNegative ? "red" : "green"]}`} onClick={toggleMonthModal}>
			<HiCalendar size={24}/>
